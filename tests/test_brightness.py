...
class TestLoadBrightness:
    def test_returns_default_when_config_missing(self, tmp_path):
        ...
        with patch("scoreboard_manager.USER_CONFIG_PATH", str(missing)):
    ...
    def test_returns_value_from_config(self, tmp_path):
        ...
        with patch("scoreboard_manager.USER_CONFIG_PATH", str(cfg)):
            result = ScoreboardManager._load_brightness(MagicMock())
        assert result == 50
    ...
    def test_returns_default_when_value_is_null(self, tmp_path):
        cfg = tmp_path / "config.json"
        cfg.write_text(json.dumps({"brightness": None}))
        with patch("scoreboard_manager.USER_CONFIG_PATH", str(cfg)):
            result = ScoreboardManager._load_brightness(MagicMock())
        assert result == 100
