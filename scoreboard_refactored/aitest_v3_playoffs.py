...
class schedule:
    ...
    def get_sched():
        ...
        while not sched and days_ahead < max_days_to_check:
            ...
            print(f"No games found, checking {date_string}...")
            sched = statsapi.schedule(start_date=date_string, team=112)

        if sched:
            print(f"Found game(s) on {date_string}")
        else:
...
def postponed(a, canvas, image, lineup, gameid, opp_image, cubs_image, batting_image):
    try:
        ...
        while current_time != time_compare or sched[a]['status'] != 'In Progress':
            ...
            for x in range(96):
                canvas.SetPixel(x, 14, 255, 255, 255)
            graphics.DrawText(canvas, warmup_font, 8,
                              12, textColor, 'POSTPONED')
            graphics.DrawText(canvas, time_font, 17, 24,
                              textColor, 'START TIME')
            graphics.DrawText(canvas, time_font, 36, 32, textColor, start_time)
            pos -= 1
    ...
    while True:
        ...
        if home_team == 112:
            if sched[a]['home_score'] >= 10:
                graphics.DrawText(canvas, cubs_score_font, 17,
                                  13, run_color, str(sched[a]['home_score']))
            else:
                graphics.DrawText(canvas, cubs_score_font, 20,
                                  13, run_color, str(sched[a]['home_score']))
            if sched[a]['away_score'] >= 10:
                graphics.DrawText(canvas, cubs_score_font, 17,
                                  30, run_color, str(sched[a]['away_score']))
            else:
                graphics.DrawText(canvas, cubs_score_font, 20,
                                  30, run_color, str(sched[a]['away_score']))
            ...
            if sched[a]['home_score'] >= 10:
                graphics.DrawText(canvas, cubs_score_font, 17,
                                  13, run_color, str(sched[a]['away_score']))
            else:
                graphics.DrawText(canvas, cubs_score_font, 20,
                                  13, run_color, str(sched[a]['away_score']))
            if sched[a]['away_score'] >= 10:
                graphics.DrawText(canvas, cubs_score_font, 17,
                                  30, run_color, str(sched[a]['home_score']))
            else:
                graphics.DrawText(canvas, cubs_score_font, 20,
                                  30, run_color, str(sched[a]['home_score']))
