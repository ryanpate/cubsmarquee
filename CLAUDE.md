...
- `nfl_preempt_mlb` — when `true`, a live NFL game takes over the display the way a live MLB game does: the run loop skips `process_game_cycle()` entirely, so no MLB content is shown until the NFL game leaves ESPN's `state: 'in'` (default `false`). The demotion to a scheduled NFL card is the *other* direction — it is what the NFL rotation slot does when this key is off and an MLB game is in progress. Football season is detected from the ESPN schedule (game within -3/+14 days), not from a month range, so preseason counts like the regular season; when that schedule is unreachable the code falls back to `extended_month_gate()` (Aug–Feb) so a network blip cannot silently hide all NFL content
